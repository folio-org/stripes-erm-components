...
const FormAccessControl = ({
  ...
  disabled, // If disabled for any external reason
  isLoading = false, // If this is loading for any external reason...
  onToggle,
  open
}) => {
  ...
  const { flattenedPolicies: claimPolicies } = useClaimPolicies({
  ...
  if (isLoading || enabledEnginesQuery.isLoading) {
    return (<Spinner />);
  }
  ...
  return (
    <Accordion
      ...
      label={<FormattedMessage id="stripes-erm-components.accesscontrol.label" />}
      onToggle={onToggle}
      open={open}
    >
      <PolicyTypedown
        input={{
          name: 'myField', // This isn't in a form so ignore the name anyway
