...
import { flattenedClaimPolicies } from '../../../../../test/jest/resources';
...
const TestComponent = () => {
  ...
  // console.log("FP: %o", flattenedClaimPolicies);
  ...
  return (
    <PolicyTypedown
      ...
      policies={[...flattenedClaimPolicies, { type: 'made-up-type', policy: { id: 'made-up-id' } }]}
    />
  );
};
...
describe('PolicyTypedown', () => {
  ...
  test('renders correct number of policy options', () => {
    ...
    expect(getAllByRole('button')).toHaveLength(flattenedClaimPolicies.length + 1);
  });

  test.each(flattenedClaimPolicies.map(cp => cp.policy.name))('%s option renders', (policyName) => {
    const { getByText } = renderComponent;
  ...
  describe.each([
    {
      filterString: '(non member)',
      expectedEntries: 4
    },
    {
      expectedEntries: 1,
      filterString: 'An acquisition unit restricting delete, user is a member'
    },
    {
      expectedEntries: 1,
      filterString: 'made-up-id'
    }])('filtering: $filterString', ({ expectedEntries, filterString }) => {
    beforeEach(async () => {
      await waitFor(async () => {
        await TextField().fillIn(filterString);
      });
    });

    test('renders correct number of policy options', async () => {
      const { getAllByRole } = renderComponent;
      await waitFor(() => {
        expect(getAllByRole('button')).toHaveLength(expectedEntries);
      });
    });
  });
});
