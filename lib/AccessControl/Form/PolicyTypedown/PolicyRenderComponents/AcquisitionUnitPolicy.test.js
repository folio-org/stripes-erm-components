...
// Use manual mock which can get these properly
jest.mock('../../../hooks/useClaimPolicies');

const TestComponent = ({ typed, isSelected }) => {
  const { flattenedClaimPolicies } = useClaimPolicies({});

  return (
    <AcquisitionUnitPolicy
      isSelected={isSelected}
      policy={flattenedClaimPolicies[0]} // Just use the first of these for this test
      typed={typed}
    />
  );
};
