...
// Mock deeply the useClaimPolicies hook
jest.mock('../../../../hooks/useRestrictionPolicies/useClaimPolicies/useClaimPolicies');

const TestComponent = ({ typed, isSelected }) => {
  const { flattenedPolicies } = useClaimPolicies({});

  // Find the one with the expected name instead of assuming it's index 0
  // necessary after implement a sort into policiesFlattener
  const targetPolicy = flattenedPolicies.find(p => p.policy.name === 'Restrict create (member)');
