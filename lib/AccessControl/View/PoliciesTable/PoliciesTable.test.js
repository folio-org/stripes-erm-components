...
describe('PoliciesTable', () => {
  describe.each([
    ...
    beforeEach(() => {
      ...
      sortedByNameAsc = orderBy(flattenedClaimPolicies, iteratee, 'asc');
      sortedByNameDesc = orderBy(flattenedClaimPolicies, iteratee, 'desc');

      renderWithIntl(
        <PoliciesTable
          allowRemove={allowRemove}
          onRemove={onRemove}
          policies={flattenedClaimPolicies}
        />,
    ...
    if (allowRemove) {
      ...
      test('removeButton renders with expected Tooltip', async () => {
        await MultiColumnListCell({ row: 0, columnIndex: 3 }).find(Tooltip(`Remove "${sortedByNameAsc[0].policy.name}"`)).has({ proximity: true });
      });
