...
describe('PoliciesTable', () => {
  describe.each([
    ...
    // Non-interactive headers: the interactor throws when trying to click them.
    // We intentionally catch that and then assert the order didn't change.
    describe('clicking non-interactive headers does not change sort', () => {
      ...
      test.each(nonInteractive)(
        ...
        async (hdr) => {
          ...
          // Still the same first row as initial ascending-by-name
          await MultiColumnListCell({ row: 0, columnIndex: 0 }).has({
            content: sortedByNameAsc[0].policy.name
          });
        }
  ...
  // Explicit case-insensitive coverage using a minimal mixed-case dataset
