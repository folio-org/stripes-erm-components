...
export default function withKiwtFieldArray(WrappedComponent) {
  class WithKiwtFieldArray extends React.Component {
    ...
    */
    ...
    handleDeleteField = (index, field, savedProps) => {
      this.props.fields.remove(index);
      this.handleMarkForDeletion(field, savedProps);
    }
    ...
    handleMarkForDeletion = (field, savedProps = []) => {
      if (field && field.id) {
        const tombstone = { id: field.id, _delete: true };

        savedProps.forEach(prop => { tombstone[prop] = field[prop]; });

        this.props.fields.push(tombstone);
      }
    }
