  ...
  <div id="tasks">
    { props.items.map((item, i) => (
      <div key={i} data-test-task>
        ...
        />
        <Button data-test-update onClick={() => props.onUpdateField(i, { updated: true })}>
          Update Task
        </Button>
...
// These tests have been turned off for the 2.3.2 hotfix release.
// They should be back _on_ in v3.x.y and later. The reason they're turned off is
// because of the React Final Form regression mentioned in ERM-825.
// The regression only affects tests so we don't need to do any version locking for RFF.
describe.skip('withKiwtFieldArray', () => {
  ...
  describe('mounted without initial values', () => {
    ...
    describe('creating a task', () => {
      ...
      describe('updating a task', () => {
        beforeEach(async () => {
          await interactor.items(0).update();
        });

        testSubmit({
          tasks: [
            { label: todo, _delete: false, updated: true },
          ]
        });
      });
  ...
  describe('mounted with three initial values', () => {
    ...
    // Skipping this until https://github.com/final-form/final-form-arrays/issues/44 is resolved.
    describe.skip('deleting the first two items and prepending two items', () => {
      beforeEach(async () => {
        await interactor.items(0).delete();
        await interactor.items(0).delete();
