...
export default class CustomPropertiesListField extends React.Component {
  ...
  isInvalid = (values) => {
    ...
    this.state.customProperties.forEach((customProperty) => {
      ...
      // Check for required field in optional section
      ...
      if ((note && !value) || (publicNote && !value)) {
        if (customProperty.type === customPropertyTypes.NUMBER || customProperty.type === customPropertyTypes.DECIMAL) {
          errors[customProperty.value] = <FormattedMessage id="stripes-erm-components.errors.customPropertyNoteInvalidNumber" />;
        } else {
          errors[customProperty.value] = <FormattedMessage id="stripes-erm-components.errors.customPropertyNoteWithoutValue" />;
        }
      }

      // number type not in range
      ...
      // decimal type not in range and limit to 2 decimal points
      if (customProperty.type === customPropertyTypes.DECIMAL) {
        const regexp = /^-?[0-9]*(\.[0-9]{0,2})?$/; // regex for a number with a maximum of 2 decimal places

        if ((value < min || value > max)) {
          errors[customProperty.value] = <FormattedMessage id="stripes-erm-components.errors.customPropertyDecimalValueNotInRange" values={{ min, max }} />;
        }

        if (value && !regexp.test(value)) {
          errors[customProperty.value] = <FormattedMessage id="stripes-erm-components.errors.customPropertyMaxTwoDecimals" />;
        }
      }

      // Check for bad input for both number and decimal type
      if (customProperty.type === customPropertyTypes.NUMBER || customProperty.type === customPropertyTypes.DECIMAL) {
        if (this.inputRef?.current?.validity?.badInput) {
          errors[customProperty.value] = <FormattedMessage id="stripes-erm-components.errors.customPropertyInvalidNumber" />;
        }
      }
    });
  ...
  renderCustomPropertyValue = (customProperty, i) => {
    ...
    if (customProperty.type === customPropertyTypes.NUMBER || customProperty.type === customPropertyTypes.DECIMAL) {
      ...
      fieldProps.inputRef = this.inputRef;
    }
