...
export default class CustomPropertiesListField extends React.Component {
  ...
  renderCustomPropertyValue = (customProperty, i) => {
    ...
    return (customProperty.type === customPropertyTypes.NUMBER || customProperty.type === customPropertyTypes.DECIMAL) ?
      (
        <IntlConsumer>
          { intl => (
            <Field
              ...
              step="any"
              type="number"
              validate={(fieldValue, allValues) => this.validate(fieldValue, allValues, customProperty)}
