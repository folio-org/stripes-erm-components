...
const CUSTOM_PROPERTY_TYPE_DECIMAL = 'com.k_int.web.toolkit.custprops.types.CustomPropertyDecimal';
  ...
  isInvalid = (values) => {
    ...
    this.state.customProperties.forEach((customProperty) => {
      ...
      if (!customProperty.primary && customProperty.value && !value) {
        if (customProperty.type === CUSTOM_PROPERTY_TYPE_NUMBER || customProperty.type === CUSTOM_PROPERTY_TYPE_DECIMAL) {
      ...
      if (customProperty.type === CUSTOM_PROPERTY_TYPE_DECIMAL) {
        const min = Number.MIN_SAFE_INTEGER;
        const max = Number.MAX_SAFE_INTEGER;
        ...
        const regexp = /^[0-9]*(\.[0-9]{0,2})?$/; // regex for number with a maximum of 2 decimal places
