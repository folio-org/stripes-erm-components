...
export default class LicenseTermsList extends React.Component {
  ...
  renderVisibility = (internal, term) => {
    if (internal === false) {
      return <span data-test-term-visibility={term.name}>{<FormattedMessage id="stripes-erm-components.term.internalFalse" />}</span>;
    }

    return <span data-test-term-visibility={term.name}>{<FormattedMessage id="stripes-erm-components.term.internalTrue" />}</span>;
  }
  ...
  renderTerm = (term, index) => {
    ...
    const internalNote = get(this.props.license, ['customProperties', term.name, '0', 'note']);
    const publicNote = get(this.props.license, ['customProperties', term.name, '0', 'publicNote']);
    const internal = get(this.props.license, ['customProperties', term.name, '0', 'internal']);
        ...
        <Row>
          ...
          <Col xs={6}>
            <KeyValue label={<FormattedMessage id="stripes-erm-components.term.publicNote" />}>
              { publicNote ? (
                <span data-test-term-publicNote={term.name}>{publicNote}</span>
              ) : '-'
              }
            </KeyValue>
          </Col>
