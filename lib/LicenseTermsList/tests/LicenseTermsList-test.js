...
describe('LicenseTermsList', () => {
  ...
  it('renders the expected visibility under the first license term', () => {
    expect(licenseTermsList.licenseTerm(0).internal).to.equal(get(license, ['customProperties', terms[0].name, '0', 'internal']));
  });

  it('renders the expected internal note under the first license term', () => {
    expect(licenseTermsList.licenseTerm(0).note).to.equal(get(license, ['customProperties', terms[0].name, '0', 'note']));
  });

  it('renders the expected public note under the first license term', () => {
    expect(licenseTermsList.licenseTerm(0).publicNote).to.equal(get(license, ['customProperties', terms[0].name, '0', 'publicNote']));
  });
  ...
  it('does not render an internal note under the second license term', () => {
    expect(licenseTermsList.licenseTerm(1).isNotePresent).to.be.false;
  });

  it('does not render a public note under the second license term', () => {
    expect(licenseTermsList.licenseTerm(1).isPublicNotePresent).to.be.false;
  });
