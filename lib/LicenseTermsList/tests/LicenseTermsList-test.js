...
describe('LicenseTermsList', () => {
  ...
  it('renders a name under a license term', () => {
    expect(licenseTermsList.licenseTerm(0).isNamePresent).to.be.true;
  });
  ...
  it('renders the expected value under the first license term', () => {
    expect(licenseTermsList.licenseTerm(0).name).to.equal(get(license, ['customProperties', terms[0].name, '0', 'value']));
  });
  ...
  it('renders the expected value under the second license term', () => {
    expect(licenseTermsList.licenseTerm(1).name).to.equal(get(license, ['customProperties', terms[1].name, '0', 'value']));
  });
  ...
  it('renders the value as Not set under the third license term', () => {
    expect(licenseTermsList.licenseTerm(2).name).to.equal('Not set');
  });
});
