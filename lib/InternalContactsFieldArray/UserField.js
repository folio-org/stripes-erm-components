  ...
  renderLinkUserButton = value => (
    <Pluggable
      ...
      renderTrigger={(props) => {
        ...
        const userString = this.props.user.personal ?  `${this.props.user.personal.lastName}${this.props.user.personal.firstName ? ', ' : ' '}${this.props.user.personal.firstName ? this.props.user.personal.firstName : ''} ${this.props.user.personal.middleName ? this.props.user.personal.middleName : ''}` : `user ${this.props.index + 1}`;
        return (
          value ? 
          <Tooltip
            text={<FormattedMessage id="stripes-erm-components.contacts.replaceUserSpecific" values={{ user: userString}} />}
            //text="Testing"
