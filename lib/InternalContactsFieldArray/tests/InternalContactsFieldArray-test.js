...
describe('InternalContactsFieldArray', () => {
  ...
  beforeEach(async () => {
    ...
    const findUserButtonComponent = props => props.renderTrigger({
      buttonRef,
      onClick: () => {
        userToSelect += 1;
    ...
    const findUserModule = getPluginModule(findUserButtonComponent, 'find-user');
      ...
      describe('clicking the add button again', () => {
        ...
        describe('linking a second user', () => {
          ...
          describe('deleting the first contact', () => {
            ...
            it('does not render the first contact\'s information in the first card', () => {
              expect(interactor.contacts(0).userPhone).to.not.equal(users[0].personal.phone);
              expect(interactor.contacts(0).userEmail).to.not.equal(users[0].personal.email);
            });

            it('renders the second contact\'s information in the first card', () => {
              expect(interactor.contacts(0).userPhone).to.equal(users[1].personal.phone);
              expect(interactor.contacts(0).userEmail).to.equal(users[1].personal.email);
            });
          });
        });
      });
