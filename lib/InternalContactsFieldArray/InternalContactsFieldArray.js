...
class InternalContactsFieldArray extends React.Component {
  ...
  handleUserSelected = /* istanbul ignore next */ (index, user) => {
    this.props.onReplaceField(index, { user: user.id });
  ...
  handleUserUnselected = (index, user) => {
    /* handleUserUnselected should mark the User to be deleted once we update the form.
    ...
    of replacing the linked User UI with the default Add User UI */
    this.props.onMarkForDeletion(user);
  ...
  renderContacts = () => {
    ...
    return items.map((contact, index) => (
      <EditCard
        ...
        deleteButtonTooltipText={<FormattedMessage id="stripes-erm-components.contacts.removeContact" />}
      ...
      >
        <Field
          ...
          onUserUnselected={() => this.handleUserUnselected(index, contact)}
