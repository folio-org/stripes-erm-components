...
class TermsConfigForm extends React.Component {
  ...
  sendCallout = (operation, outcome, error = '') => {
    this.callout.sendCallout({
      type: outcome,
      message: <FormattedMessage id={`stripes-erm-components.terms.callout.${operation}.${outcome}`} values={{ error }} />,
  ...
  sendCalloutTermInUse = () => {
    this.callout.sendCallout({
      type: 'error',
      message: <SafeHTMLMessage id="stripes-erm-components.terms.callout.delete.termInUse" />,
      timeout: 0,
    });
  }
  ...
  render() {
    ...
    return (
      <Pane
        defaultWidth="fill"
        id="settings-terms"
        paneTitle={<FormattedMessage id="stripes-erm-components.section.terms" />}
        paneSub={<FormattedMessage id="stripes-erm-components.terms.termCount" values={{ count }} />}
      >
        <form>
          <FieldArray
