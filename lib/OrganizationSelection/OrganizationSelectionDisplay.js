...
export default class OrganizationSelectionDisplay extends React.Component {
  ...
  render() {
    ...
    return (
      <FormattedMessage id="erm-stripes-components.orgSelect.selectOrg">
        {placeholder => (
          <Selection
            dataOptions={organizations}
            emptyMessage={!searchString ? <FormattedMessage id="erm-stripes-components.orgSelect.typeToSearch" /> : undefined}
            error={error}
            formatter={(props) => {
              const { option } = props;
