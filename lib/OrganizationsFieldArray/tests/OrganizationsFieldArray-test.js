...
describe.only('OrganizationsFieldArray', () => {
  setupApplication();
  ...
  describe('clicking the add button', () => {
    ...
    describe('setting the role field', () => {
      ...
      it('has a role field count of 1', () => {
        expect(interactor.countRoles).to.equal(1);
      });

      it('has no role delete button', () => {
        // expect(interactor.roles(0).hasDeleteRoleButton).to.be.false;
        expect(interactor.hasDeleteRoleButton).to.be.false;
      });
    });
    ...
    describe('adding a second role', () => {
      beforeEach(async () => {
        await interactor.clickAddRoleButton();
      });

      it('has a role field count of 2', () => {
        expect(interactor.countRoles).to.equal(2);
      });

      it('has a role delete button', () => {
        // expect(interactor.roles(0).hasDeleteRoleButton).to.be.true;
        expect(interactor.hasDeleteRoleButton).to.be.true;
      });
    });

    describe('clicking the delete role button', () => {
      beforeEach(async () => {
        await interactor.clickAddRoleButton();
        await interactor.clickDeleteRoleButton();
      });

      it('removes a role field', () => {
        expect(interactor.countRoles).to.equal(1);
      });
    });
