...
const EditOrganizationCard = ({
  ...
  const nameOfOrgNotLinked = useRef(null);
  ...
  useEffect(() => {
    const orgUuids = values?.orgs?.filter(o => o?.org?.orgsUuid !== undefined).map(filteredOrg => filteredOrg?.org?.orgsUuid) || [];

    if (uniq(orgUuids).length !== orgUuids.length && (index + 1) >= orgUuids.length) {
      setDuplicateWarning(true);
      nameOfOrgNotLinked.current = input?.value?.org?.name;
      change(`${input.name}.org`, undefined);
    }
  }, [change, duplicateWarning, index, input, nameOfOrgNotLinked, setDuplicateWarning, values]);
  ...
  return (
    ...
    >
      <Field
        ...
        duplicateMessage={<FormattedMessage id="stripes-erm-components.organizations.duplicateOrgError" values={{ orgName: nameOfOrgNotLinked.current }} />}
