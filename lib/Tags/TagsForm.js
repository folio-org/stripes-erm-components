...
export default class TagsForm extends React.Component {
  ...
  onChange(tags) {
    ...
    if (tags.length < this.state.entityTags.length) {
      const tag = difference(this.state.entityTags.map(t => t.value.toLowerCase()), tags.map(t => t.value.toLowerCase()));
  ...
  render() {
    ...
    const tagsList = entityTags.map(tag => ({ value: tag.value.toLowerCase(), label: tag.value.toLowerCase() }));
    const addAction = { onSelect: this.addTag, render: this.renderTag };
    const actions = [addAction];
    return (
      <FormattedMessage id="stripes-erm-components.enterATag">
        {placeholder => (
          <FormattedMessage id="stripes-erm-components.tagsTextArea">
            {ariaLabel => (
              <MultiSelection
