...
describe('Tags', () => {
  ...
  test('renders empty application ', () => {
    const { getByRole } = renderComponent;
    expect(getByRole('application', { name: '' })).toBeInTheDocument();
  });

  test('renders expected search with zero value selected', () => {
    const { getByRole } = renderComponent;
    expect(getByRole('search', { name: '0 items selected' })).toBeInTheDocument();
  });
