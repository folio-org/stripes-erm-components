...
const DocumentFilterField = ({
  ...
  const [focusRuleIndex, setFocusRuleIndex] = useState(null);
  ...
  const renderRuleComponent = (ruleFields, ruleFieldName, ruleFieldIndex) => {
    return (
      <DocumentFilterRule
        ...
        shouldFocusAttribute={(ruleFieldIndex === focusRuleIndex)}
  ...
  return (
    <>
      ...
      <Button
        ...
        onClick={() => {
          const nextRuleIndex = values.filters[index]?.rules?.length || 0;
          push(`${name}.rules`, {});
          setFocusRuleIndex(nextRuleIndex);
        }}
