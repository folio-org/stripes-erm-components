...
const DocumentFilter = ({ activeFilters, atTypeValues, filterHandlers }) => {
  ...
  const parsedFilterData = parseQueryString(activeFilters?.supplementaryDocuments || []);
  ...
  return (
    <Accordion
      ...
      id="clickable-agreement-suppl-document-filter"
      label={<FormattedMessage id="stripes-erm-components.documentFilter.supplementaryDocuments" />}
      onClearFilter={() => filterHandlers.state({ ...activeFilters, supplementaryDocuments: [] })
      }
      separator={false}
    >
