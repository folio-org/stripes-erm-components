...
describe('DocumentFilter', () => {
  describe.each([
    [
      'with active filters',
      { docs: ['(docs.name==testing)'] },
      '1 document filters applied',
      [{ rules: [{ comparator: '==', path: 'docs.name', value: 'testing' }] }]
    ],
    [
    ...
    [
      ...
      { docs: ['(docs.name==testing&&docs.note!~wibble)||(docs.atType.value!=license)'] },
      '2 document filters applied',
      [{ rules: [{ comparator: '==', path: 'docs.name', value: 'testing' }, { comparator: '!~', path: 'docs.note', value: 'wibble' }] }, { rules: [{ comparator: '!=', path: 'docs.atType.value', value: 'license' }] }]
    ],
  ])('ActiveFilter tests', (
    activeFilterTitle,
    activeFilterState,
