...
import { useInterfaceCredentials } from '../hooks';
...
const ViewOrganizationCard = ({
  ...
  const toggleCredentials = toggleId => {
    ...
    if (!storedInterfaceCredentials[toggleId]) {
      fetchCredentials(toggleId); // FIXME We should be able to remove this eventually
      setInterfaceId(toggleId);
    }
  };

  useEffect(() => {
    if (interfaceCredentials?.id && !!storedInterfaceCredentials[interfaceId?.id]) {
      setStoredInterfaceCredentials({
        ...storedInterfaceCredentials,
        [interfaceId?.id]: interfaceCredentials
      });
    }
  }, [interfaceCredentials, interfaceId?.id, storedInterfaceCredentials]);

  console.log("INTERFACE CREDENTIALS DATA: %o", interfaceCredentials);
  ...
  const renderInterfaces = () => {
    return interfaces.length ? (
      <div data-test-interfaces data-testid="interfaces">
        <MultiColumnList
          columnMapping={{
  ...
  const renderRoles = () => (
    roles.map(r => {
      const { rnote, role: { label, value } } = r;
      return (
  ...
  return (
    <div>
      <Card
        ...
        roundedBorder
      >
        { roles?.length && <KeyValue label={<FormattedMessage id="stripes-erm-components.organizations.roles" />}>{renderRoles(roles)}</KeyValue> }
        { note &&
      ...
      }
        {
          interfaces ?
            renderInterfaces() :
            (hasOrgData ? renderOrganizationName(org.name) : renderEmpty())
        }
      </Card>
    </div>
  );
