...
export default class ViewOrganizationCard extends React.Component {
  ...
  renderEmpty = () => {
    const { duplicateMessage, duplicateWarning, meta, setDuplicateWarning } = this.props;
    ...
    return (
      <div>
        <Card
          ...
          roundedBorder
        >
          { roles?.length && <KeyValue label={<FormattedMessage id="stripes-erm-components.organizations.roles" />}>{this.renderRoles(roles)}</KeyValue> }
          { note &&
