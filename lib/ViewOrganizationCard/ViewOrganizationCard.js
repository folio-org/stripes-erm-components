...
const ViewOrganizationCard = ({
  ...
  const renderOrganizationStatus = () => {
    if (org?.orgsUuid_object?.status) {
      return `&nbsp;·&nbsp;${org.orgsUuid_object.status}`;
    }
    return undefined;
  };
  ...
  const renderCard = () => {
    ...
    if (hasOrgData) {
      const status = org?.orgsUuid_object?.status ? ` · ${org.orgsUuid_object.status}` : '';
      return (
        <>
          {renderOrganizationName(`${org.name}${status}`)}
        </>
      );
    }
