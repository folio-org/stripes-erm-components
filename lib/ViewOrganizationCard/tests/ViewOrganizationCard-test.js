...
describe('ViewOrganizationCard', () => {
  ...
  it('password is hidden', () => {
    expect(interactor.creds(0).passwordBlockText).to.contain('****');
  });

  it('username is hidden', () => {
    expect(interactor.creds(0).usernameBlockText).to.contain('****');
  });

  describe('Click the show credentials button', () => {
    beforeEach(async () => {
      await interactor.creds(0).showCredsButton();
    });

    it('calls getCreds', () => {
      expect(getCreds).to.be.called();
    });
  });
