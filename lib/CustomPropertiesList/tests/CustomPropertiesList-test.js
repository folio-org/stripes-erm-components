...
import CustomPropertiesListInteractor from './interactor';
...
describe('CustomPropertiesList', () => {
  ...
  it('renders the expected value under the first license term', () => {
    expect(customPropertiesList.licenseTerm(0).value).to.equal(get(license, ['customProperties', terms[0].name, '0', 'value']));
  });

  it('renders the expected internal note under the first license term', () => {
    expect(customPropertiesList.licenseTerm(0).note).to.equal(get(license, ['customProperties', terms[0].name, '0', 'note']));
  });

  it('renders the expected public note under the first license term', () => {
    expect(customPropertiesList.licenseTerm(0).publicNote).to.equal(get(license, ['customProperties', terms[0].name, '0', 'publicNote']));
  });
  ...
  it('renders the expected value under the second license term', () => {
    expect(customPropertiesList.licenseTerm(1).value).to.equal(get(license, ['customProperties', terms[1].name, '0', 'value']));
  });
