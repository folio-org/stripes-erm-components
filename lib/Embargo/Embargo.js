...
export default class Embargo extends React.Component {
    ...
    renderEmbargoEnd(embargo, alignment) {
    ...
    renderEmbargoStart(embargo, alignment) {
    ...
    render() {
      const { alignLeft, embargo } = this.props;
      if (!embargo) return null;

      const alignment = alignLeft ? 'flex' : 'flex centerContent';

      return (
        <Layout className="full" data-test-embargo>
          {this.renderEmbargoStart(embargo, alignment)}
          {this.renderEmbargoEnd(embargo, alignment)}
        </Layout>
      );
    }
}
