...
export default class InnerCard extends React.Component {
  static propTypes = {
  ...
  renderOrganizationPicker = (index) => (
    <OrganizationPicker onAddOrg={this.onAddOrg} index={index} />
  );
  ...
  renderInnerCardDefault = () => {
    ...
    return (
      <Field
        ...
        headerEnd={this.renderOrganizationPicker(index)}
        roundedBorder
      >
        <React.Fragment>
  ...
  render() {
    ...
    return org && !isEmpty(org) ?
      this.renderInnerCardInterfaces() :
      this.renderInnerCardDefault();
  }
}
