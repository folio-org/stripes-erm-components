...
export default class TermsListField extends React.Component {
  ...
  renderTermName = (term, i) => {
    ...
    return (
      <Select
        ...
        id={`edit-term-${i}-name`}
        label={<FormattedMessage id="stripes-erm-components.prop.termName" />}
        onChange={e => {
  ...
  isInvalid = (values) => {
    ...
    this.state.terms.forEach((term) => {
      ...
      if ((note && !value) || (publicNote && !value)) {
        errors[term.value] = <FormattedMessage id="stripes-erm-components.errors.termNoteWithoutValue" />;
      }
      ...
      if (term.type === TERM_TYPE_NUMBER) {
        ...
        if (value < min || value > max) {
          errors[term.value] = <FormattedMessage id="stripes-erm-components.errors.termValueNotInRange" values={{ min, max }} />;
        }
      }
    });
  ...
  renderTermValue = (term, i) => {
    ...
    return (
      <FieldComponent
        ...
        label={<FormattedMessage id="stripes-erm-components.prop.termValue" />}
  ...
  renderTermVisibility = (term, i) => {
    ...
    const dataOptions = (intl) => {
      return [
        { value: true, label: intl.formatMessage({ id: 'stripes-erm-components.term.internalTrue' }) },
        { value: false, label: intl.formatMessage({ id: 'stripes-erm-components.term.internalFalse' }) }
      ];
    };
      ...
      <IntlConsumer>
        {intl => (
          <Select
            ...
            label={<FormattedMessage id="stripes-erm-components.prop.termVisibility" />}
            onChange={handleChange}
  ...
  renderTermNoteInternal = (term, i) => {
    ...
    return (
      <TextArea
        ...
        label={<FormattedMessage id="stripes-erm-components.term.internalNote" />}
        onChange={handleChange}
        value={note}
      />
    );
  ...
  renderTermNotePublic = (term, i) => {
    ...
    return (
      <TextArea
        ...
        onChange={handleChange}
        label={<FormattedMessage id="stripes-erm-components.term.publicNote" />}
        value={publicNote}
      />
    );
  }
  ...
  renderAddTerm = () => {
    return (
      ...
      >
        <FormattedMessage id="stripes-erm-components.terms.add" />
      </Button>
    );
  }
  ...
  renderTerms = (termType) => {
    ...
    const termsList = this.state.terms.map((term, i) => {
      ...
      const header = termType === 'optional' ?
        <FormattedMessage id="stripes-erm-components.term.title" values={{ number: optionalTermCounter + 1 }} /> :
        term.label;
      ...
      return (
        <EditCard
          ...
          deleteButtonTooltipText={<FormattedMessage id="stripes-erm-components.term.removeTerm" />}
          header={header}
          key={term.value}
